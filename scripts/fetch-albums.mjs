...
async function findAlbum({ artist, title }) {
  ...
  const hit =
    data.results.find((r) =>
      r.collectionName
        .toLowerCase()
        .startsWith(title.toLowerCase().slice(0, 8)),
    ) ?? data.results[0];
...
for (const album of ALBUMS) {
  ...
  for (let i = 0; i < tracks.length; i++) {
    ...
    if (downloadTrack(album.artist, tracks[i].name, dest)) {
      ...
      if (!durationMs) {
        ...
        durationMs =
          Math.round(parseFloat(probe.stdout?.toString() ?? "0") * 1000) ||
          240000;
      }
  ...
  manifest.albums.sort(
    (a, b) =>
      ALBUMS.findIndex((x) => x.id === a.id) -
      ALBUMS.findIndex((x) => x.id === b.id),
  );
