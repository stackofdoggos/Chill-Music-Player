...
| `public/models/room.glb` → `area_rug` | Jute area rug under the desk: corrugated slab + border rope, materials from `public/textures/rug/` (custom baked braid tile). Click-treated with floor in `SceneModel.tsx`. |
